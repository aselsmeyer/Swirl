...
| omit all columns from X through size (X:size). To see how this might work, let's look at a numerical example with -5:20.
...
| entire sequence of numbers from 5 through 20, so that we get -5, -6, -7, ... , -18, -19, -20. Try the same thing, except
| surround 5:20 with parentheses so that R knows we want it to first come up with the sequence of numbers, then apply the
...
| Now that you know how to select a subset of columns using select(), a natural next question is "How do I select a subset
| of rows?" That's where the filter() function comes in.
...
| The == operator asks whether the thing on the left is equal to the thing on the right. If yes, then it returns TRUE. If
| no, then FALSE. In this case, package is an entire vector (column) of values, so package == "swirl" returns a vector of
| TRUEs and FALSEs. filter() then returns only the rows of cran corresponding to the TRUEs.
...
| request rows for which EITHER one condition OR another condition are TRUE. For example, filter(cran, country == "US" |
                                                                                                   | country == "IN") will gives us all rows for which the country variable equals either "US" or "IN". Give it a go.
...
| This is another tricky one. filter(cran, !is.na(r_version)) will return all rows of cran for which r_version is NOT NA.
...
Error in "size":"ip_id" : NA/NaN argument
...
| will first arrange by package names (ascending alphabetically), then by ip_id. This means that if there are multiple
| rows with the same value for package, they will be sorted by ip_id (ascending numerically). Try arrange(cran2, package,
| ip_id) now.
...
| The size variable represents the download size in bytes, which are units of computer memory. These days, megabytes (MB)
| are a more common unit of measurement. One megabyte is equal to 2^20 bytes. That's 2 to the power of 20, which is
...
| third column, all in the same line of code. To see this in action, repeat the exact same command as above, except add a
| third argument creating a column that is named size_gb and equal to size_mb / 2^10.
...
| Let's try one more for practice. Pretend we discovered a glitch in the system that provided the original values for the
| size variable. All of the values in cran3 are 1000 bytes less than they should be. Using cran3, create just one new
...
| knowing the average download size. summarize(cran, avg_bytes = mean(size)) will yield the mean value of the size
| variable. Here we've chosen to label the result 'avg_bytes', but we could have named it anything. Give it a try.

> summarize(cran, avg_bytes - mean(size))
Error: object 'avg_bytes' not found
> summarize(cran, avg_bytes = mean(size))
...
| In this lesson, you learned how to manipulate data using dplyr's five main functions. In the next lesson, we'll look at
| how to take advantage of some other useful features of dplyr to make your life as a data analyst much easier.
...
Selection: 1
What is your email address? aselsmeyer@yahoo.com
What is your assignment token? XSUT7gDvsekvdlqT
...
| this lesson. Normally, this is something you would have to do on your own. Just to build the habit, type library(dplyr)
...
| function and store the result in a variable called cran. If you're not sure what I'm talking about, you should start
...
| package variable. Everything else looks the same, but now any operation we apply to the grouped data will take place on
| a per package basis.
...
| of all values in the size column. We may care about what that number is, but wouldn't it be so much more interesting to
| look at the mean download size for each unique package?
...
| number of distinct ip_id's. The 'countries' column, created with n_distinct(country), provides the number of countries
| in which each package was downloaded. And finally, the 'avg_bytes' column, created with mean(size), contains the mean
...
| 'count' is strictly greater (>) than 679. Store the result in a new variable called top_counts.
...
| arrange() the rows of top_counts based on the 'count' column and assign the result to a new variable called top_counts_sorted. We want
| the packages with the highest number of downloads at the top, which means we want 'count' to be in descending order. If you need help,
| check out ?arrange and/or ?desc.

> top_counts_soted <- arrange(top_counts, count)

| Not quite, but you're learning! Try again. Or, type info() for more options.

| arrange(top_counts, desc(count)) will arrange the rows of top_counts based on the values of the 'count' variable, in
| descending order. Don't forget to assign the result to top_counts_sorted.
...
| I've opened up a script that contains code similar to what you've seen so far. Don't change anything. Just study it for
| a minute, make sure you understand everything that's there, then submit() when you are ready to move on.
